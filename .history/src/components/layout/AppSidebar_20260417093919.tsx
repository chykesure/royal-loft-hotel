...
export function AppSidebar() {
  ...
  // super_admin and developer see everything
  const visibleItems = menuItems.filter((item) => {
    const role = user?.role;
    if (role === 'super_admin' || role === 'developer') return true;

    const modulePerms = permissions[item.permModule];
    // If no permissions data yet, show everything (safe default during load)
    if (!modulePerms) return true;

    // Only show if user has at least 'view' permission for the module

...
export function StaffPayrollModule() {
  ...
  const handleBatchAttendance = async (status: string) => {
    ...
    const target = status === 'remove'
      ? currentStaff
      : currentStaff.filter(s => !currentAttendance[s.id]);
        ...
        <CardContent>
          ...
          <div className="flex items-center justify-between mt-2">
            <p className="text-xs text-muted-foreground">
              Showing {filteredStaff.length} of {safe.staff.length} staff
            </p>
            <p className="text-xs text-muted-foreground">
          ...
          {safe.payrollRecords.length > 0 && (
            <div className="flex flex-wrap gap-4 mb-4 p-3 bg-muted/50 rounded-lg text-sm">
              <div>
                ...
                <span className="font-semibold">{formatCurrency(safe.totalPayroll)}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Paid: </span>
                <span className="font-semibold text-green-600">{formatCurrency(safe.paidPayroll)}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Outstanding: </span>
                <span className="font-semibold text-amber-600">
                  {formatCurrency(safe.totalPayroll - safe.paidPayroll)}
                </span>
              </div>
              <div>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="border rounded-lg overflow-hidden">
              <div className="bg-muted/50 p-2 text-xs text-muted-foreground flex justify-between">
                <span>{safe.staff.length} active staff profiles</span>
            ...
            {safe.staff.some(s => s.baseSalary > 0) && (
              <div className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">
                <span className="font-medium">
                  Total Payroll ({safe.staff.filter(s => s.baseSalary > 0).length} staff)
                </span>
            ...
            {safe.staff.length > 0 && !safe.staff.some(s => s.baseSalary > 0) && (
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={generating || !safe.staff.some(s => s.baseSalary > 0)}
            >
              {generating ? (
                <>

...
const emptyLinkForm: LinkUserForm = {
  userId: '',
  userName: '',
  employeeId: '',
  department: '',
...
export function StaffPayrollModule() {
  ...
  // Unassigned users (without StaffProfile)
  ...
  const fetchUnassigned = useCallback(async () => {
    try {
      ...
      const res = await fetch('/api/staff?mode=unassigned');
      if (res.ok) {
        const data = await res.json();
  ...
  const openLinkDialog = (user: UnassignedUser) => {
    setLinkForm({
      userId: user.id,
      userName: user.name,
      employeeId: '',
      department: user.department || 'front_desk',
      position: user.role === 'admin' || user.role === 'manager' ? 'Manager' : 'Staff',
      baseSalary: '',
      bankName: '',
      bankAccount: '',
    });
  ...
  const handleLinkUser = async () => {
    if (!linkForm.userId || !linkForm.employeeId || !linkForm.department || !linkForm.position || !linkForm.baseSalary) {
      toast.error('Please fill in all required fields');
      return;
    }
    setIsLinking(true);
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        toast.success(`${linkForm.userName} is now on the staff list!`);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 shrink-0 ml-2">
                          {member.status !== 'terminated' && member.status !== 'suspended' && (
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                className={`h-6 w-6 p-0 text-xs ${attStatus === 'present' ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'text-emerald-600 border-emerald-300'}`}
                                disabled={isLoadingAtt}
                                onClick={(e) => { e.stopPropagation(); handleAttendance(member.id, 'present'); }}
                              ...
                              <Button
                                ...
                                className={`h-6 w-6 p-0 text-xs ${attStatus === 'on_leave' ? 'bg-orange-500 hover:bg-orange-600 text-white' : 'text-orange-600 border-orange-300'}`}
                                disabled={isLoadingAtt}
                                onClick={(e) => { e.stopPropagation(); handleAttendance(member.id, 'on_leave'); }}
                              ...
                              <Button
                                ...
                                className={`h-6 w-6 p-0 text-xs ${attStatus === 'absent' ? 'bg-red-500 hover:bg-red-600 text-white' : 'text-red-600 border-red-300'}`}
                                disabled={isLoadingAtt}
                                onClick={(e) => { e.stopPropagation(); handleAttendance(member.id, 'absent'); }}
                          ...
                          <Badge className={`text-[10px] px-2 py-0.5 h-5 border-0 ${ATTENDANCE_BADGE[attStatus] || ''}`}>
            ...
            <CardContent>
              <div className="space-y-3">
                {payrollByDepartment.map((dept) => {
                  ...
                  return (
                    <div key={dept.department} className="p-3 rounded-lg bg-muted/30">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{dept.count} staff</p>
                          </div>
                        </div>
                      ...
                      <div className="h-1.5 w-full rounded-full bg-muted overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${DEPT_ACCENT[dept.department] || 'bg-gray-400'}`}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-amber-50 border border-amber-200">
                  <div>
                    ...
                    <p className="text-xs text-amber-600">{summary?.totalActive ?? 0} active staff</p>
                  </div>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  <div className="p-3 rounded-lg bg-muted/30 text-center">
                    ...
                    <p className="text-sm font-semibold text-emerald-600">
                      {staff.filter((s) => s.currentPayroll && s.currentPayroll.status === 'paid').length} of {summary.totalStaff}
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted/30 text-center">
                    <p className="text-xs text-muted-foreground">Pending</p>
                    <p className="text-sm font-semibold text-orange-600">
                      {staff.filter((s) => s.currentPayroll && s.currentPayroll.status === 'pending').length} of {summary.totalStaff}
                    </p>
                  </div>
                </div>
              )}
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-base flex items-center gap-2">
              ...
              {unassignedUsers.length > 0 && (
                <Badge className="bg-orange-100 text-orange-700 border-0 text-[10px] px-1.5 py-0 h-5">
            ...
            <CardContent>
              {isLoadingUnassigned ? (
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-muted-foreground">All users have staff profiles</p>
                </div>
              ) : (
                  ...
                  {unassignedUsers
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="h-9 w-9 rounded-full bg-orange-500 flex items-center justify-center text-white text-xs font-semibold shrink-0">
                          {user.name.split(' ').map((n) => n[0]).join('').slice(0, 2)}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-medium truncate">{user.name}</p>
                          <p className="text-xs text-muted-foreground truncate">
                            {user.email} {user.role ? `· ${user.role}` : ''} {user.department ? `· ${DEPARTMENT_LABELS[user.department] || user.department}` : ''}
                          </p>
                        </div>
                      </div>
                      <Button
                        size="sm"
                        className="bg-amber-500 hover:bg-amber-600 text-white h-8 text-xs shrink-0"
                        onClick={() => openLinkDialog(user)}
                      >
                        <UserCheck className="h-3.5 w-3.5 mr-1" /> Create Profile
                      </Button>
                    </div>
                  ))}
                </div>
      ...
      <Dialog open={addStaffOpen} onOpenChange={setAddStaffOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Create a new staff member profile with their role and salary details.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <Label className="text-xs text-muted-foreground">Full Name *</Label>
              <Input placeholder="e.g. John Doe" value={addForm.name} onChange={(e) => setAddForm({ ...addForm, name: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Email *</Label>
              <Input type="email" placeholder="staff@royalloft.com" value={addForm.email} onChange={(e) => setAddForm({ ...addForm, email: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Phone</Label>
              <Input placeholder="08012345678" value={addForm.phone} onChange={(e) => setAddForm({ ...addForm, phone: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Employee ID *</Label>
              <Input placeholder="e.g. RL-001" value={addForm.employeeId} onChange={(e) => setAddForm({ ...addForm, employeeId: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Department *</Label>
              <Select value={addForm.department} onValueChange={(val) => setAddForm({ ...addForm, department: val })}>
            ...
            <div>
              ...
              <Input placeholder="e.g. Receptionist" value={addForm.position} onChange={(e) => setAddForm({ ...addForm, position: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Base Salary (₦) *</Label>
              <Input type="number" placeholder="e.g. 150000" value={addForm.baseSalary} onChange={(e) => setAddForm({ ...addForm, baseSalary: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Bank Name</Label>
              <Input placeholder="e.g. GTBank" value={addForm.bankName} onChange={(e) => setAddForm({ ...addForm, bankName: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Bank Account</Label>
              <Input placeholder="Account number" value={addForm.bankAccount} onChange={(e) => setAddForm({ ...addForm, bankAccount: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Password</Label>
              <Input type="password" placeholder="Default: RoyalLoft@123" value={addForm.password} onChange={(e) => setAddForm({ ...addForm, password: e.target.value })} className="mt-1" />
            </div>
          </div>
          <DialogFooter className="gap-2 sm:gap-0 mt-2">
            <Button variant="outline" onClick={() => setAddStaffOpen(false)}>Cancel</Button>
            <Button className="bg-amber-500 hover:bg-amber-600 text-white" onClick={handleAddStaff} disabled={isAdding}>
              {isAdding ? (<><Loader2 className="h-4 w-4 mr-1 animate-spin" /> Adding...</>) : (<><UserPlus className="h-4 w-4 mr-1" /> Add Staff</>)}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Create Staff Profile for {linkForm.userName}
            </DialogTitle>
            <DialogDescription>This will create a staff profile linked to the existing user account.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2 p-3 rounded-lg bg-muted/30">
              <p className="text-sm font-medium">{linkForm.userName}</p>
              <p className="text-xs text-muted-foreground">Existing user account — will be linked to this staff profile</p>
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Employee ID *</Label>
              <Input placeholder="e.g. RL-005" value={linkForm.employeeId} onChange={(e) => setLinkForm({ ...linkForm, employeeId: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Department *</Label>
              <Select value={linkForm.department} onValueChange={(val) => setLinkForm({ ...linkForm, department: val })}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Select department" /></SelectTrigger>
                <SelectContent>
                  {DEPARTMENTS.map((d) => (<SelectItem key={d.value} value={d.value}>{d.label}</SelectItem>))}
                </SelectContent>
            ...
            <div>
              <Label className="text-xs text-muted-foreground">Position *</Label>
              <Input placeholder="e.g. Receptionist" value={linkForm.position} onChange={(e) => setLinkForm({ ...linkForm, position: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Base Salary (₦) *</Label>
              <Input type="number" placeholder="e.g. 150000" value={linkForm.baseSalary} onChange={(e) => setLinkForm({ ...linkForm, baseSalary: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Bank Name</Label>
              <Input placeholder="e.g. GTBank" value={linkForm.bankName} onChange={(e) => setLinkForm({ ...linkForm, bankName: e.target.value })} className="mt-1" />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Bank Account</Label>
              <Input placeholder="Account number" value={linkForm.bankAccount} onChange={(e) => setLinkForm({ ...linkForm, bankAccount: e.target.value })} className="mt-1" />
            </div>
          </div>
          <DialogFooter className="gap-2 sm:gap-0 mt-2">
      ...
      <Dialog open={generateOpen} onOpenChange={setGenerateOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Set deductions per staff member. Base salary will be used for net pay calculation.</DialogDescription>
          </DialogHeader>
          <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
            {staff.filter((s) => s.status === 'active').map((member) => (
              <div key={member.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                <div className="flex items-center gap-2 min-w-0">
                  <div className={`h-7 w-7 rounded-full ${DEPT_ACCENT[member.department] || 'bg-gray-500'} flex items-center justify-center text-white text-[10px] font-semibold shrink-0`}>
                    {member.user.name.split(' ').map((n) => n[0]).join('').slice(0, 2)}
                  </div>
                  <div className="min-w-0">
                    <p className="text-sm font-medium truncate">{member.user.name}</p>
                    <p className="text-xs text-muted-foreground">Base: {formatCurrency(member.baseSalary)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Input
                    ...
                    onChange={(e) => setPayrollDeductions({ ...payrollDeductions, [member.id]: e.target.value })}
          ...
          <DialogFooter className="gap-2 sm:gap-0 mt-2">
            ...
            <Button className="bg-violet-500 hover:bg-violet-600 text-white" onClick={handleGeneratePayroll} disabled={isGenerating}>
              {isGenerating ? (<><Loader2 className="h-4 w-4 mr-1 animate-spin" /> Generating...</>) : (<><CalendarCheck className="h-4 w-4 mr-1" /> Generate Payroll</>)}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Pay Salary — {payTarget?.user.name}
            </DialogTitle>
            <DialogDescription>
              {payTarget?.currentPayroll && (
                <span className="text-sm font-semibold text-emerald-700">
                  Net Pay: {formatCurrency(payTarget.currentPayroll.netPay)}
                </span>
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            {payTarget?.currentPayroll && (
              <div className="p-3 rounded-lg bg-muted/30 space-y-1.5 text-xs">
                <div className="flex justify-between"><span className="text-muted-foreground">Basic Salary</span><span>{formatCurrency(payTarget.currentPayroll.basicSalary)}</span></div>
                {payTarget.currentPayroll.overtimePay > 0 && (
                  <div className="flex justify-between"><span className="text-muted-foreground">Overtime</span><span className="text-emerald-600">+{formatCurrency(payTarget.currentPayroll.overtimePay)}</span></div>
                )}
                {payTarget.currentPayroll.bonus > 0 && (
                  <div className="flex justify-between"><span className="text-muted-foreground">Bonus</span><span className="text-emerald-600">+{formatCurrency(payTarget.currentPayroll.bonus)}</span></div>
                )}
                {payTarget.currentPayroll.deductions > 0 && (
                  <div className="flex justify-between"><span className="text-muted-foreground">Deductions</span><span className="text-red-600">-{formatCurrency(payTarget.currentPayroll.deductions)}</span></div>
                )}
                {payTarget.currentPayroll.taxAmount > 0 && (
                  <div className="flex justify-between"><span className="text-muted-foreground">Tax</span><span className="text-red-600">-{formatCurrency(payTarget.currentPayroll.taxAmount)}</span></div>
                )}
                <div className="flex justify-between font-bold text-sm pt-1 border-t"><span>Net Pay</span><span className="text-emerald-700">{formatCurrency(payTarget.currentPayroll.netPay)}</span></div>
              </div>
            )}
            <div>
          ...
          <DialogFooter className="gap-2 sm:gap-0 mt-2">
            ...
            <Button className="bg-emerald-500 hover:bg-emerald-600 text-white" onClick={handlePaySalary} disabled={isPaying}>
              {isPaying ? (<><Loader2 className="h-4 w-4 mr-1 animate-spin" /> Processing...</>) : (<><Banknote className="h-4 w-4 mr-1" /> Pay Now</>)}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Staff Details — {selectedStaff?.user.name}
            </DialogTitle>
          </DialogHeader>
          {selectedStaff && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-muted/30">
                  ...
                  <p className="text-sm font-medium">{selectedStaff.employeeId}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
                  <p className="text-xs text-muted-foreground">Department</p>
                  <p className="text-sm font-medium">{DEPARTMENT_LABELS[selectedStaff.department] || selectedStaff.department}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
                ...
                <div className="p-3 rounded-lg bg-muted/30">
                  ...
                  <Badge className={`text-[10px] px-2 py-0.5 h-5 border-0 ${STATUS_BADGE[selectedStaff.status] || ''}`}>
                    {formatLabel(selectedStaff.status)}
                  </Badge>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
                  <p className="text-xs text-muted-foreground">Base Salary</p>
                  <p className="text-sm font-semibold">{formatCurrency(selectedStaff.baseSalary)}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
                  <p className="text-xs text-muted-foreground">Start Date</p>
                  <p className="text-sm font-medium">{new Date(selectedStaff.startDate).toLocaleDateString()}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 rounded-lg bg-muted/30">
                  ...
                  <p className="text-sm font-medium truncate">{selectedStaff.user.email}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/30">
              ...
              <div className="p-3 rounded-lg bg-muted/30">
                ...
                {selectedStaff.todayAttendance ? (
                  <div className="flex items-center gap-4">
                    <Badge className={`text-[10px] px-2 py-0.5 h-5 border-0 ${ATTENDANCE_BADGE[selectedStaff.todayAttendance.status] || ''}`}>
                      {formatLabel(selectedStaff.todayAttendance.status)}
                    </Badge>
                    {selectedStaff.todayAttendance.clockIn && (
                      <span className="text-xs text-muted-foreground">In: {new Date(selectedStaff.todayAttendance.clockIn).toLocaleTimeString()}</span>
                    )}
                    {selectedStaff.todayAttendance.clockOut && (
                      <span className="text-xs text-muted-foreground">Out: {new Date(selectedStaff.todayAttendance.clockOut).toLocaleTimeString()}</span>
                    )}
                    {selectedStaff.todayAttendance.hoursWorked && (
                      <span className="text-xs text-muted-foreground">{selectedStaff.todayAttendance.hoursWorked}h</span>
                    )}
                  </div>
                ) : (
              ...
              {selectedStaff.currentPayroll && (
                <div className="p-3 rounded-lg bg-muted/30">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs text-muted-foreground">Current Payroll ({getCurrentPeriod()})</p>
                    <Badge className={`text-[10px] px-2 py-0.5 h-5 border-0 ${selectedStaff.currentPayroll.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-700'}`}>
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    {selectedStaff.currentPayroll.deductions > 0 && (<div className="flex justify-between"><span className="text-muted-foreground">Deductions</span><span className="text-red-600">-{formatCurrency(selectedStaff.currentPayroll.deductions)}</span></div>)}
                    {selectedStaff.currentPayroll.taxAmount > 0 && (<div className="flex justify-between"><span className="text-muted-foreground">Tax</span><span className="text-red-600">-{formatCurrency(selectedStaff.currentPayroll.taxAmount)}</span></div>)}
                    <div className="flex justify-between font-bold text-sm pt-1 border-t"><span>Net</span><span>{formatCurrency(selectedStaff.currentPayroll.netPay)}</span></div>
                  </div>
                  {selectedStaff.currentPayroll.status === 'pending' && (
                    <Button size="sm" className="bg-emerald-500 hover:bg-emerald-600 text-white mt-3 w-full" onClick={() => { openPayDialog(selectedStaff); setDetailOpen(false); }}>
                      <Banknote className="h-3.5 w-3.5 mr-1" /> Pay Salary
                    </Button>
                  )}
                </div>
              )}

              {/* Pay salary button if no payroll yet */}
              {!selectedStaff.currentPayroll && (
                <Button size="sm" variant="outline" className="w-full" onClick={() => { setGenerateOpen(true); setDetailOpen(false); }}>
                  <CalendarCheck className="h-3.5 w-3.5 mr-1" /> Generate Payroll First
                </Button>
              )}
            </div>
          )}

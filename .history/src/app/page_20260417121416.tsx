...
import { ExpensesModule } from '@/components/skeletons/';
...
// NOTE: blacklist values use MODULE_PERM_MAP values (e.g. 'developer_tools' NOT 'developer-tools')
...
export default function Home() {
  const { isAuthenticated, isLoading, setUser, setPermissions, user, permissions } = useAuthStore();
  ...
  useEffect(() => {
    if (!user || !permissions || Object.keys(permissions).length === 0) return;
    const role = user.role;
    // Only super_admin and developer can access all modules
    if (role === 'developer') return;

    // Enforce hard blacklist first (applies to super_admin too)
    const permModule = MODULE_PERM_MAP[currentModule];
    const blacklist = ROLE_MODULE_BLACKLIST[role || ''];
    if (blacklist && blacklist.includes(permModule)) {
      setCurrentModule('dashboard');
      return;
    }

    // super_admin skips the dynamic permission check
    if (role === 'super_admin') return;

    // Then check dynamic permissions
    const modulePerms = permissions[permModule];
    if (!modulePerms || !modulePerms.view) {
      setCurrentModule('dashboard');
    }
  }, [user, permissions, currentModule, setCurrentModule]);

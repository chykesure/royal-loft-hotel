  ...
  useEffect(() => {
    const initApp = async () => {
      try {
        ...
        const rbacPromise = init().catch(() => { /* non-critical */ });
    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto relative z-0">
          {/* Render all modules but only show the active one.
              This prevents re-mounting and re-fetching data on every module switch. */}
          {Object.entries(moduleComponents).map(([key, Component]) => (
            <div
              key={key}

...
export async function POST(request: NextRequest) {
  try {
    ...
    let recentFailedAlerts: { createdAt: Date }[] = [];
    try {
      recentFailedAlerts = await db.securityAlert.findMany({
        where: {
          type: 'failed_login',
          userId: user.id,
      ...
      // SecurityAlert table may not exist yet — skip lockout check
    }
    ...
    if (!isValid) {
      ...
      try {
        ...
        // SecurityAlert table may not exist — skip logging
      }
    ...
    // Successful login — clear any existing failed login alerts (non-blocking)
    ...
    // Initial session: 30 minutes idle timeout (refreshed on every API call)
    ...
    try {
      ...
      // AuditLog table may not exist — skip logging
    }
    ...
    response.cookies.set('auth_token', token, {
      ...
      sameSite: 'lax',
      maxAge: 30 * 60, // 30 minutes (matches server idle timeout)
      path: '/',
    });

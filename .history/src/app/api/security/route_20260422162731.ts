...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'create-user') {
      ...
      const validRoles = ['super_admin', 'developer', 'manager', 'front_desk', 'housekeeping', 'accountant', 'auditor', 'staff'];
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (action === 'update-user') {
      ...
      if (body.role && !['super_admin', 'developer'].includes(session.user.role)) {
    ...
    if (action === 'change-password') {
      ...
      // Only allow users to change their own password, or admin/developer to change any
      if (session.user.id !== id && !['super_admin', 'developer'].includes(session.user.role)) {

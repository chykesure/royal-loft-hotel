...
// Only super_admin and developer can manage users
function canManageUsers(userRole: string): boolean {
  return userRole === 'developer' || userRole === 'super_admin';
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (section === 'users') {
      // Only admin/developer can view user list
    ...
    // Return current user's own info for non-admin
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'create-user') {
      ...
      if (!canManageUsers(user.role)) {
        return NextResponse.json({ error: 'Access denied. Only Super Admin and Developer can create users.' }, { status: 403 });
      }
      ...
      return NextResponse.json({ id: newUser.id, email: newUser.email, name: newUser.name, role: newUser.role }, { status: 201 });
    }
    ...
    if (action === 'save-permissions') {
      if (!canManageUsers(user.role)) {
        return NextResponse.json({ error: 'Access denied. Only Super Admin and Developer can modify permissions.' }, { status: 403 });
      }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (action === 'update-user') {
      ...
      if (!canManageUsers(user.role)) {
        return NextResponse.json({ error: 'Access denied. Only Super Admin and Developer can edit users.' }, { status: 403 });
      }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!canManageUsers(user.role)) {
      return NextResponse.json({ error: 'Access denied. Only Super Admin and Developer can delete users.' }, { status: 403 });
    }

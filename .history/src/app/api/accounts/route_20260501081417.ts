...
export async function GET(request: NextRequest) {
  try {
    ...
    const monthlyExpenses = await db.expense.findMany({
      ...
      select: { amount: true, total: true } as any,
    } as any);

    const totalExpenses = monthlyExpenses.reduce((sum: number, e: any) => sum + (e.total || e.amount || 0), 0);
    ...
    try {
      ...
      const prevExpenseTotal = prevExpenses.reduce((sum: number, e: any) => sum + (e.total || e.amount || 0), 0);

...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalExpenses = monthlyExpenses.reduce((sum: number, e: any) => sum + (e.amount || e.total || 0), 0);
    ...
    try {
      ...
      const prevExpenseTotal = prevExpenses.reduce((sum: number, e: any) => sum + (e.amount || e.total || 0), 0);
...
function buildPaymentMethodsBreakdown(payments: any[]) {
  ...
  for (const p of payments) {
    ...
    if (methodMap[method]) {
      ...
      const existing = methodMap[method];
      methodMap[method] = {
        total: (existing?.total || 0) + (p.amount || 0),

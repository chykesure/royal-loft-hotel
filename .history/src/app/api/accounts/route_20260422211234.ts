...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      totalBilled = monthlyBills.reduce((s, b) => s + b.totalAmount, 0);
      totalCollected = monthlyBills.reduce((s, b) => s + b.paidAmount, 0);
      roomRevenue = monthlyBills.reduce((s, b) => s + b.roomCharges, 0);
      foodRevenue = monthlyBills.reduce((s, b) => s + b.foodCharges, 0);
      barRevenue = monthlyBills.reduce((s, b) => s + b.barCharges, 0);
      spaRevenue = monthlyBills.reduce((s, b) => s + b.spaCharges, 0);
      laundryRevenue = monthlyBills.reduce((s, b) => s + b.laundryCharges, 0);
      otherRevenue = monthlyBills.reduce((s, b) => s + b.otherCharges, 0);
      totalDiscounts = monthlyBills.reduce((s, b) => s + b.discountAmount, 0);
    } catch (err) {
    ...
    try {
      ...
      outstandingBills = outstandingResult._sum.balanceAmount || 0;
    } catch (err) {
    ...
    try {
      ...
      monthlyPayroll = payrollRecords.reduce((s, p) => s + p.netPay, 0);
      processedPayroll = payrollRecords.filter(p => p.status === 'paid' || p.status === 'processed').reduce((s, p) => s + p.netPay, 0);
      pendingPayroll = payrollRecords.filter(p => p.status === 'pending').reduce((s, p) => s + p.netPay, 0);
    } catch (err) {
    ...
    try {
      ...
      console.error('[ACCOUNTS] Payment method query failed:', err);
    }
    ...
    try {
      ...
      console.error('[ACCOUNTS] Daily revenue query failed:', err);
    }
    ...
    try {
      ...
      console.error('[ACCOUNTS] Income transactions query failed:', err);
    }
    ...
    try {
      ...
      console.error('[ACCOUNTS] Expense transactions query failed:', err);
    }

...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      totalExpenses = monthlyExpenses.reduce((sum: number, e: any) => sum + (e.total || e.amount || 0), 0);
    } catch {
      // Expense table or columns may not exist yet
    }
    ...
    try {
      ...
      try {
        ...
        prevExpenseTotal = prevExpenses.reduce((sum: number, e: any) => sum + (e.total || e.amount || 0), 0);
      } catch {
        // ignore
      }
    ...
    let recentExpenseRecords: any[] = [];
    try {
      recentExpenseRecords = await db.expense.findMany({
        take: 20,
        orderBy: { date: 'desc' },
      } as any);
    } catch {
      // Expense table may not exist
    }

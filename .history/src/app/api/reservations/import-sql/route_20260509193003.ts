...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let batchStart = 0; batchStart < bookingRows.length; batchStart += BATCH_SIZE) {
      ...
      for (let i = batchStart; i < batchEnd; i++) {
        ...
        try {
          ...
          // In the legacy system, total_price is ALREADY the discounted amount.
          // The actual room amount was: total_price + discount.
          // netPaid = what the guest actually paid = total_price (discount already applied)
          const netPaid = totalPrice;
          const originalAmount = totalPrice + discount;
          if (netPaid <= 0) {
            skipped++;
          ...
          // roomRate = original rack rate per night (before discount)
          const roomRate = Math.round((originalAmount / nights) * 100) / 100;
      ...
      // Phase 2: Execute all DB writes in a single transaction
          ...
          for (const op of batchOps) {
            ...
            const reservation = await tx.reservation.create({
              data: {
                ...
                notes: `Imported from legacy system (Booking #${op.bookingNum}, original: ₦${op.totalPrice + op.discount}, discount: ₦${op.discount})`,
            ...
            const bill = await tx.bill.create({
              data: {
                ...
                guestId: op.guestId,
                // roomCharges = the original rack rate amount (total_price + discount)
                roomCharges: op.totalPrice + op.discount,
                discountAmount: op.discount,
                // totalAmount = what was actually paid (total_price, already discounted)
                totalAmount: op.netPaid,
                paidAmount: op.netPaid,
                balanceAmount: 0,
    ...
    const totalRevenue = bookingRows.reduce((sum, row) => {
      // total_price is already the discounted amount paid
      return sum + cleanNum(row[6]);
    }, 0);

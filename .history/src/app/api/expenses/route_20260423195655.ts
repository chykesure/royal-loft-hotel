...
export async function GET(request: NextRequest) {
  try {
    // Try auth but don't block if Session table doesn't exist
    let user: { id: string; name: string; role: string } | null = null;
    try {
      user = await authenticate();
    } catch {
      // Session table might not exist — allow read-only access
    }
    ...
    try {
      ...
      // Aggregate may fail if table doesn't exist
    }
...
export async function POST(request: NextRequest) {
  try {
    let user: { id: string; name: string; role: string } | null = null;
    try { user = await authenticate(); } catch { /* Session table issue */ }
    if (!user) {
    ...
    const { category, description, amount, vendor, reference } = body;

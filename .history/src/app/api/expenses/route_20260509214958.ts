...
// ── GET: List expenses with optional filters (excludes salary expenses) ──
export async function GET(request: NextRequest) {
  try {
    ...
    const where: any = { category: { not: 'salaries' } };
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { date, category, description, amount, vendor, reference } = body;
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, date, category, description, amount, vendor, reference } = body;
